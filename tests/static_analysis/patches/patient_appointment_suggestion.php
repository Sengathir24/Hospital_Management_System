<?php
/*
Suggested remediation for patient/appointment.php queries that interpolate $userid

Original pattern (example):
    $sqlmain = "... where patient.pid=$userid ";

Suggested approach: build base SQL and bind parameters.

Example snippet to replace the interpolation and dynamic filtering:
    // --- BEGIN SUGGESTED SNIPPET ---
    $base_sql = "SELECT appointment.appoid, schedule.scheduleid, schedule.title, doctor.docname, patient.pname, schedule.scheduledate, schedule.scheduletime, appointment.apponum, appointment.appodate FROM schedule INNER JOIN appointment ON schedule.scheduleid=appointment.scheduleid INNER JOIN patient ON patient.pid=appointment.pid INNER JOIN doctor ON schedule.docid=doctor.docid WHERE patient.pid = ?";
...
Notes: validate date input and any other user-provided data before binding.
*/
